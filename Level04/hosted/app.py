...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Check for invalid characters in the encrypted message
        encrypted_message = params['encrypted'][0]
        if any(not char.isdigit() and char not in [' ', '\t'] for char in encrypted_message):
            self._set_response()
            error_message = "Invalid characters in the encrypted message."
            self.wfile.write(f'''
                <html>
                <body>
